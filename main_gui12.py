...
DEPLOYMENT_STATE_FILE = Path(os.environ.get("CARLA_DEPLOYMENT_STATE_FILE", "logs/deployment_state.json"))
DEPLOYMENT_STATE_STALE_SEC = float(os.environ.get("CARLA_DEPLOYMENT_STATE_STALE_SEC", "300.0"))
...
DEPLOYMENT_STOP_STREAM_SERVICES = env_flag("DEPLOYMENT_STOP_STREAM_SERVICES", "1")
POST_DEPLOY_STREAM_START_DELAY_SEC = float(os.environ.get("POST_DEPLOY_STREAM_START_DELAY_SEC", "8.0"))
COMMAND_START_BUFFER_SEC = float(os.environ.get("COMMAND_START_BUFFER_SEC", "3.0"))
COMMAND_SWITCH_COOLDOWN_SEC = float(os.environ.get("COMMAND_SWITCH_COOLDOWN_SEC", "20.0"))
CARLA_VIDEO_SERVICE = os.environ.get("CARLA_VIDEO_SERVICE", "carla-video.service")
...
CENTER_DISPLAY_AUTO_START = env_flag("CENTER_DISPLAY_AUTO_START", "1")
...
_deployment_state_lock = threading.Lock()
_last_deployment_finished_at = 0.0


def _deployment_state_payload(active, step=None, msg=None, ok=None, body=None):
    payload = {
        "active": bool(active),
        "pid": os.getpid(),
    ...
    if body is not None:
        payload["command"] = {
            ...
            if key in {"scene", "sky", "sunshinetime", "drive_mode", "loadingtransportation", "vehiclemodel", "camera_view", "sendstate"}
        }
    return payload


def write_deployment_state(active, step=None, msg=None, ok=None, body=None):
    try:
        with _deployment_state_lock:
            DEPLOYMENT_STATE_FILE.parent.mkdir(parents=True, exist_ok=True)
            DEPLOYMENT_STATE_FILE.write_text(
                json.dumps(_deployment_state_payload(active, step, msg, ok, body), ensure_ascii=False, indent=2),
                encoding="utf-8",
            )
    except Exception as exc:
        print(f"deployment state write skipped: {exc}")


def touch_deployment_state(step=None, msg=None):
    lock = globals().get("deployment_lock")
    if lock is not None and lock.locked():
        write_deployment_state(True, step=step, msg=msg)
...
def read_deployment_state():
    ...
    if active:
        ...
        stale = updated_ts <= 0.0 or (time.time() - updated_ts) > DEPLOYMENT_STATE_STALE_SEC
    ...
    lock = globals().get("deployment_lock")
    data["active"] = bool(data.get("active")) or bool(lock is not None and lock.locked())
    return data
...
def systemctl_user(action, *units, timeout=8):
    if os.name == "nt" or not units:
        return
    try:
        subprocess.run(
        ...
        print(f"systemctl --user {action} {' '.join(units)} skipped: {exc}")


def pause_stream_services_for_deploy():
    if not DEPLOYMENT_STOP_STREAM_SERVICES:
        return
    # Mirror cameras are CARLA clients too; keep them out while maps and actors are rebuilt.
    systemctl_user("stop", CENTER_DISPLAY_SERVICE, CARLA_VIDEO_SERVICE, timeout=10)


def resume_stream_services_after_deploy():
    if not DEPLOYMENT_STOP_STREAM_SERVICES:
        ensure_center_display_started()
        return
    if POST_DEPLOY_STREAM_START_DELAY_SEC > 0:
        time.sleep(POST_DEPLOY_STREAM_START_DELAY_SEC)
    systemctl_user("start", CARLA_VIDEO_SERVICE, timeout=10)
    ensure_center_display_started()
...
class SimulationState:
    ...
    def set_last_pipeline_result(self, ok, msg):
        ...
        if not ok:
            touch_deployment_state("failed", msg)
    ...
    def set_pipeline_step(self, step, msg=None):
        ...
        touch_deployment_state(step, msg or step)
...
def force_realtime_world(world, client=None, reason=""):
    """Keep CARLA asynchronous, but use a fixed physics step for stable dynamics."""
    if world is None:
        return
    try:
        ...
        if getattr(settings, "synchronous_mode", False):
            ...
            changed = True
        fixed_delta = float(os.environ.get("OFFICIAL_FIXED_DELTA_SECONDS", "0.02"))
        if getattr(settings, "fixed_delta_seconds", None) != fixed_delta:
            settings.fixed_delta_seconds = fixed_delta
            changed = True
        if getattr(settings, "no_rendering_mode", False):
            settings.no_rendering_mode = False
            changed = True
        if not getattr(settings, "substepping", False):
            settings.substepping = True
            changed = True
        substep_delta = float(os.environ.get("OFFICIAL_MAX_SUBSTEP_DELTA_TIME", "0.005"))
        if getattr(settings, "max_substep_delta_time", None) != substep_delta:
            settings.max_substep_delta_time = substep_delta
            changed = True
        max_substeps = int(os.environ.get("OFFICIAL_MAX_SUBSTEPS", "16"))
        if getattr(settings, "max_substeps", None) != max_substeps:
            settings.max_substeps = max_substeps
            changed = True
        if changed:
            world.apply_settings(settings)
            print(f"CARLA world forced to asynchronous realtime mode{f' ({reason})' if reason else ''}.")
        if client is not None:
            try:
                client.get_trafficmanager(TM_PORT).set_synchronous_mode(False)
            except Exception as e:
                print(f"TrafficManager async mode set failed: {e}")
    except Exception as e:
        print(f"force realtime world failed: {e}")


def build_legacy_algo_telemetry(raw_telemetry, vehicle_transform, vehicle_velocity, ui_params):
    """Build the local 127.0.0.1:5000 payload consumed by AIGO scripts."""
    payload = dict(raw_telemetry or {})
    rigid_key = "1_刚体运动学 (Rigid Body Kinematics)"
    dyn_key = "6_动态车辆参数"

    rigid = dict(payload.get(rigid_key) or {})
    try:
        rigid.setdefault("1_全局绝对坐标_XYZ_米", [
            round(vehicle_transform.location.x, 3),
            round(vehicle_transform.location.y, 3),
            round(vehicle_transform.location.z, 3),
        ])
        rigid.setdefault("2_姿态角_俯仰_偏航_滚转_度", [
            round(vehicle_transform.rotation.pitch, 3),
            round(vehicle_transform.rotation.yaw, 3),
            round(vehicle_transform.rotation.roll, 3),
        ])
        rigid.setdefault("3_线速度矢量_XYZ_米每秒", [
            round(vehicle_velocity.x, 3),
            round(vehicle_velocity.y, 3),
            round(vehicle_velocity.z, 3),
        ])
    except Exception:
        pass
    payload[rigid_key] = rigid

    mass = float((ui_params or {}).get("mass", 1500.0) or 1500.0)
    geom = vehicle_geometry_params()
    wheelbase = float((ui_params or {}).get("wheelbase", geom["wheelbase"]) or geom["wheelbase"])
    a_dist = float((ui_params or {}).get("a", round(wheelbase * 0.517, 3)) or round(wheelbase * 0.517, 3))
    b_dist = float((ui_params or {}).get("b", round(wheelbase * 0.483, 3)) or round(wheelbase * 0.483, 3))
    cf = float((ui_params or {}).get("cf", -110000.0 * (mass / 1500.0)) or -110000.0)
    cr = float((ui_params or {}).get("cr", -95000.0 * (mass / 1500.0)) or -95000.0)
    max_steer = float((ui_params or {}).get("steer", 40.0) or 40.0)

    dyn = dict(payload.get(dyn_key) or {})
    dyn.update({
        "整备质量": mass,
        "前轮侧偏刚度_Cf": cf,
        "后轮侧偏刚度_Cr": cr,
...
def mark_runtime_stale_after_carla_disconnect(reason):
    lock = globals().get("deployment_lock")
    if lock is not None and lock.locked():
        print(f"CARLA reconnect cleanup suppressed during deployment: {reason}")
        touch_deployment_state("carla_busy", f"CARLA temporarily unavailable during deployment: {reason}")
        return
    try:
        runtime_cleanup_simulation()
    except Exception as e:
...
def ensure_carla_connection():
    try:
        if sim_state.client is not None and sim_state.world is not None:
            try:
                sim_state.world = sim_state.client.get_world()
                force_realtime_world(sim_state.world, sim_state.client, reason="ensure existing connection")
        ...
        force_realtime_world(world, client, reason="ensure new connection")
...
def build_spawn_candidates(world, map_base_name):
    candidates = [build_spawn_transform(map_base_name)]
    try:
        fixed = candidates[0]
        for forward in (4.0, -4.0, 8.0, -8.0, 12.0):
            candidates.append(offset_transform(fixed, forward=forward, up=0.3))
        for right in (2.5, -2.5, 5.0, -5.0):
            candidates.append(offset_transform(fixed, right=right, up=0.3))
    except Exception:
        pass
    try:
        spawn_points = list(world.get_map().get_spawn_points())
        if spawn_points:
            anchor = candidates[0].location
            spawn_points.sort(key=lambda sp: sp.location.distance(anchor))
            candidates.extend(spawn_points[:40])
    except Exception:
        pass
    return candidates


def spawn_ego_vehicle_with_retry(world, bp_lib, preferred_bp_id, map_base_name):
    bp_ids = [preferred_bp_id]
    for fallback in ("vehicle.lincoln.mkz_2020", "vehicle.lincoln.mkz2017"):
        if fallback != preferred_bp_id:
            bp_ids.append(fallback)

    candidates = build_spawn_candidates(world, map_base_name)
    last_error = None
    for bp_id in ordered_unique(bp_ids):
        try:
            bp = bp_lib.find(bp_id)
            ...
            if bp.has_attribute("color"):
                vals = bp.get_attribute("color").recommended_values
                if vals:
                    bp.set_attribute("color", vals[0])
        except Exception as exc:
            last_error = f"{bp_id}: blueprint not found ({exc})"
            continue

        for index, tf in enumerate(candidates[:50], start=1):
            try:
                vehicle = world.try_spawn_actor(bp, tf)
                if vehicle is not None:
                    if bp_id != preferred_bp_id:
                        print(f"ego blueprint fallback: {preferred_bp_id} -> {bp_id}")
                    if index > 1:
                        print(f"ego spawn fallback point used: attempt={index}")
                    return vehicle, bp_id, None
            except Exception as exc:
                last_error = f"{bp_id}: {exc}"
            try:
                world.wait_for_tick()
            except Exception:
                time.sleep(0.05)
    return None, None, last_error or f"all spawn attempts failed for {preferred_bp_id}"
...
def spawn_runtime_vehicle(v_data, map_base_name, drive_mode_str, ui_params):
    ...
    vehicle, used_bp_id, spawn_error = spawn_ego_vehicle_with_retry(world, bp_lib, bp_id, map_base_name)
    if vehicle is None:
        return False, f"车辆生成失败: {spawn_error or bp_id}"
    ...
    return True, f"仿真环境已全自动生成，ego={used_bp_id or bp_id}，UDP 推流激活！"
...
def run_frontend_pipeline(body: dict):
    global _last_deployment_finished_at
    ...
    write_deployment_state(True, step="accepted", msg="command accepted, deployment running", body=body)
    try:
        wait_sec = max(0.0, (_last_deployment_finished_at + COMMAND_SWITCH_COOLDOWN_SEC) - time.time())
        wait_sec = max(wait_sec, COMMAND_START_BUFFER_SEC)
        if wait_sec > 0:
            sim_state.set_pipeline_step("command_buffer", f"buffering command for {wait_sec:.1f}s before touching CARLA")
            time.sleep(wait_sec)
        sim_state.set_pipeline_step("pause_streams", "pausing mirror/camera clients before CARLA deployment")
        pause_stream_services_for_deploy()
        ...
        sim_state.target_ip = "127.0.0.1" if is_algo_mode(mode_str) else body.get("_client_ip", sim_state.target_ip)
        ...
        force_realtime_world(sim_state.world, sim_state.client, reason=f"deploy {runtime_map}")
        ...
        final_result = sim_state.snapshot().get("last_pipeline_result") or {}
        write_deployment_state(False, step=final_result.get("step"), msg=final_result.get("msg"), ok=final_result.get("ok"))
        resume_stream_services_after_deploy()
        _last_deployment_finished_at = time.time()
        try:
...
class FrontendApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            if self.path == "/telemetry":
                deployment_state = read_deployment_state()
                self.send_json(200, {
                    "ok": True,
                    "target_ip": snap["target_ip"],
                    "drive_mode": snap["drive_mode"],
                    "deployment_active": deployment_state.get("active", False),
            ...
            if self.path != "/health":
                ...
                return
            carla_backend_online = bool(snap["carla_connected"])
            deployment_state = read_deployment_state()
            self.send_json(200, {
                "ok": True,
                "api": "running",
                ...
                "deployment_active": deployment_state.get("active", False),
                "deployment_state": deployment_state,
                "carla_backend_online": carla_backend_online,
                "carla_backend_indicator": "green" if carla_backend_online else "red",
                "carla_backend_label": "CARLA 后端运行中" if carla_backend_online else "CARLA 后端断开",
    ...
    def do_POST(self):
        ...
        try:
            ...
            response_view = sim_state.set_camera_view(CarlaEncoder.first_value(
                body,
                "camera_view",
                "Camera View",
                "view",
                ...
                "camera",
                default=sim_state.camera_view,
            )) if has_view_command else sim_state.camera_view
            response_snap = sim_state.snapshot()
            sim_state.set_last_pipeline_result(True, "command accepted, deployment running")
            write_deployment_state(True, step="queued", msg="command queued for background deployment", body=body)
...
def master_simulation_loop(client, vehicle_actor, sensors_list, stop_event, dyn_wrapper):
    ...
    ui_update_counter = 0
    ctrl_rx_count = 0
    legacy_telem_logged = False
    try:
        world = client.get_world()
        manual_tick_required = bool(getattr(world.get_settings(), "synchronous_mode", False))
        if manual_tick_required:
            print("CARLA world is synchronous; master loop will tick physics manually.")
    except Exception:
        world = None
        manual_tick_required = False
    ...
    try:
        while not stop_event.is_set():
            try:
                ...
                if dyn_wrapper:
                    ...
                    try: 
                        ...
                        if is_algo_mode(current_mode):
                            legacy_payload = build_legacy_algo_telemetry(
                                legacy_telem_data,
                                vehicle_actor.get_transform(),
                                vehicle_actor.get_velocity(),
                                dyn_wrapper.ui if dyn_wrapper else {},
                            )
                            telem_sock.sendto(
                                json.dumps(legacy_payload, ensure_ascii=False).encode("utf-8"),
                                ALGO_TELEMETRY_ADDR,
                            )
                            if not legacy_telem_logged:
                                print("Algo legacy telemetry active: 127.0.0.1:5000")
                                legacy_telem_logged = True
                    except Exception as e:
                        if ui_update_counter % 200 == 0:
                            print(f"telemetry send recovered: {e}")
                ...
                print(f"simulation loop recovered: {e}")
                time.sleep(0.05)
            if manual_tick_required and world is not None:
                try:
                    world.tick()
                except Exception as e:
                    print(f"manual world.tick failed, switching to passive loop: {e}")
                    manual_tick_required = False
