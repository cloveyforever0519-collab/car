...
class LaneChangeController:
    def __init__(self):
        ...
        self.max_steer_angle = math.radians(40.0)  # CARLA 前轮最大物理偏角
        ...
        self.prev_ed = 0.0
        self.filtered_ed_dot = 0.0

        self.start_pose = None
        self.is_finished = False
        self.count = 0
        self.param_logged = False
    ...
    def control_step(self):
        ...
        try:
            ...
            if dyn_params:
                self.Cf = dyn_params.get("前轮侧偏刚度_Cf", dyn_params.get("Cf", self.Cf))
                self.Cr = dyn_params.get("后轮侧偏刚度_Cr", dyn_params.get("Cr", self.Cr))
                self.L = dyn_params.get("轴距_L", dyn_params.get("轮距_L", self.L))
                self.a = dyn_params.get("a", self.a)
                self.b = dyn_params.get("b", self.b)
                steer_deg = dyn_params.get("最大前轮转角_deg", math.degrees(self.max_steer_angle))
                self.max_steer_angle = math.radians(max(1.0, float(steer_deg)))
                if not self.param_logged:
                    print(
                        f"动态参数: L={self.L:.3f}, a={self.a:.3f}, b={self.b:.3f}, "
                        f"Cf={self.Cf:.1f}, Cr={self.Cr:.1f}, max_steer={math.degrees(self.max_steer_angle):.1f}deg"
                    )
                    self.param_logged = True
            # ---------------------------------------------------
        ...
        # 横向控制 (LQR简化版/PID)
        now = time.time()
        dt = max(0.005, min(now - self.last_time, 0.08))
        self.last_time = now
        self.ed_integral = np.clip(self.ed_integral + ed * dt, -1.0, 1.0)
        raw_ed_dot = (ed - self.prev_ed) / dt
        self.prev_ed = ed
        self.filtered_ed_dot = 0.7 * self.filtered_ed_dot + 0.3 * raw_ed_dot
        # 1. 计算反馈项 (PID)
        feedback = self.kp_ed * ed + self.ki_ed * self.ed_integral + self.kd_ed * self.filtered_ed_dot + self.kp_ephi * ephi
        ...
        # CARLA VehicleControl.steer 接收的是最大前轮转角比例，不是弧度。
        ...
        self.count += 1
        if self.count % 20 == 0:
            print(f"速度: {curr_v * 3.6:.1f} km/h | 偏差: {ed:.2f}m | steer: {final_steer:.3f} | X: {local_x:.1f}m")
