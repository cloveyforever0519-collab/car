...
CARLA_UDP_RX_PORT = 5002
LOCAL_COMMAND_URL = "http://127.0.0.1:8765/command"
LOCAL_COMMAND_TIMEOUT = 180
FRONTEND_COMMAND_FIELDS = {"scene", "sky", "sunshinetime", "drive_mode", "loadingtransportation", "vehiclemodel"}
GATEWAY_ID = "carla_backend_gateway"
...
def build_delivery_payload(raw_data, car_name):
    if isinstance(raw_data, dict) and "Vehiclemodel" in raw_data and "Overall" in raw_data:
        return raw_data
    ...
    # --- 3. 按前端 C# 字段名封装英文 Payload，避免安卓端强类型解析错位 ---
    payload = {
        "Vehiclemodel": str(car_name),
        "Overall": str(params.get("外廓尺寸 (m)", "")),
        "Wheelbase": str(params.get("轴距 (m)", "")),
        "Tirebase": str(params.get("轮距 (m)", "")),
        "Tireradius": str(params.get("轮胎半径 (m)", "")),
        "Empty": str(params.get("空载重量 (kg)", "")),
        "Gravity": str(params.get("重心 / 质心 (X,Y,Z)", "")),
        "Axle": str(params.get("簧载质量 (kg)", "")),
        "Unloaded": str(params.get("簧下质量 (kg)", "")),
        "Moment": str(params.get("转动惯量 (Ixx,Iyy,Izz)", "")),
        "Ratedtotalmass": str(params.get("额定总质量 (kg)", "")),
        "Drag": str(params.get("风阻系数 Cd", "")),
        "Windward": str(params.get("迎风面积 (m²)", "")),
        "LiftCoefficientCl": str(params.get("升力系数 Cl", "")),
        "Pitching": str(params.get("俯仰力矩系数 Cm", "")),
        "Latera": str(params.get("侧风稳定性系数 Cy", params.get("侧风稳定性 Cy", ""))),
        
        "LeftFrontWheelRotation": f"{round(wheel_rpms[0], 1)} RPM",
        "RightFrontWheelRotation": f"{round(wheel_rpms[1], 1)} RPM",
        "LeftRearWheelRotation": f"{round(wheel_rpms[2], 1)} RPM",
        "RightRearWheelRotation": f"{round(wheel_rpms[3], 1)} RPM",
        
        "LeftFrontWheelVibration": f"{f_fl} N",
        "RightFrontWheelVibration": f"{f_fr} N",
        "LeftRearWheelVibration": f"{f_rl} N",
        "RightRearWheelVibration": f"{f_rr} N",
        
        "LeftFrontWheel": str(slip_fl),
        "RightFrontWheel": str(slip_fr),
        "LeftRearWheel": str(slip_rl),
        "RightRearWheel": str(slip_rr),
        
        "RadialDeformationLeftFrontWheel": str(def_fl),
        "RadialDeformationRightFrontWheel": str(def_fr),
        "RadialDeformationLeftRearWheel": str(def_rl),
        "RadialDeformationRightRearWheel": str(def_rr),
        
        "PitchAngle": f"{round(pitch, 2)} deg",
        "RollAngle": f"{round(roll, 2)} deg",
        "LateralSwingAngle": f"{round(yaw, 2)} deg",
        
        "LongitudinalDisplacement": f"{round(pos[0], 2)} m",
        "LateralDisplacement": f"{round(pos[1], 2)} m",
        "VerticalDisplacement": f"{round(pos[2], 2)} m",
        
        "TurningTheSteeringWheel": f"{steer_deg} deg",
        "LeftFrontWheelAngle": f"{ackermann_deg} deg",
        "RightFrontWheelAngle": f"{ackermann_deg} deg",
        "SteeringColumnTorsion": f"{round(steer_deg * 0.05, 1)} deg",
        "EngineCrankshaftRotates": f"{engine_rpm} RPM",
        "LeftDriveHalfShaftTwist": f"{round(throttle * 2.5, 1)} deg",
        "RightDriveHalfShaftTwist": f"{round(throttle * 2.5, 1)} deg"
    }
    return payload

def extract_frontend_command(data):
    """识别安卓大屏六字段指令，兼容 data/payload/command 包裹。"""
    if not isinstance(data, dict):
        return None

    candidates = [data]
    for key in ("data", "payload", "command", "body"):
        nested = data.get(key)
        if isinstance(nested, dict):
            candidates.append(nested)

    for item in candidates:
        if FRONTEND_COMMAND_FIELDS.issubset(item.keys()):
            return {field: str(item.get(field, "")).strip() for field in FRONTEND_COMMAND_FIELDS}
    return None

def sync_display_state(command):
    SysState.CURRENT_CAR = command.get("vehiclemodel") or SysState.CURRENT_CAR
    sky = command.get("sky", "")
    sun_time = command.get("sunshinetime", "")
    SysState.CURRENT_WEATHER = f"{sky} / {sun_time}".strip(" /") or SysState.CURRENT_WEATHER
    SysState.TRAFFIC_STATE = "已加载" if str(command.get("loadingtransportation")) == "0" else "未加载"
    SysState.IS_STREAMING = True

def post_local_command(command):
    ...
    try:
        with urllib.request.urlopen(req, timeout=LOCAL_COMMAND_TIMEOUT) as resp:
        ...
        result.setdefault("http_status", e.code)
        result.setdefault("msg", f"本机后端返回 HTTP {e.code}")
        return result
    except Exception as e:
        return {"ok": False, "msg": f"无法连接本机后端 {LOCAL_COMMAND_URL}: {e}"}
...
async def gateway_service():
    ...
    try:
        udp_rx.bind(("0.0.0.0", CARLA_UDP_RX_PORT))
    except OSError:
        logging.error(f"❌ {CARLA_UDP_RX_PORT} 端口被占用！请确认后台没有挂起的僵尸进程。")
        return
        ...
        try:
            ...
            async with websockets.connect(WS_URI, ping_interval=20, ping_timeout=20) as ws:
                    ...
                    try:
                        async for msg in ws:
                            try:
                                ...
                                frontend_command = extract_frontend_command(cmd_data)
                                if frontend_command:
                                    sync_display_state(frontend_command)
                                    logging.info(f"📥 收到六字段指令，转发本机后端: {frontend_command}")
                                    result = await loop.run_in_executor(None, post_local_command, frontend_command)
                                    logging.info(f"📤 本机后端执行结果: {result}")
                                    continue
                                ...
                                if "START" in cmd:
                                    ...
                                    SysState.IS_STREAMING = False
                                else:
                                    udp_tx.sendto(msg.encode('utf-8'), (CARLA_UDP_TX_HOST, CARLA_UDP_TX_PORT))
                    except websockets.exceptions.ConnectionClosed as e:
                        logging.warning(f"🔌 云端主动断开: code={e.code}, reason={e.reason!r}")
                        raise

                tasks = [
                    asyncio.create_task(udp_consumer(), name="udp_consumer"),
                    asyncio.create_task(ws_producer(), name="ws_producer"),
                    asyncio.create_task(ws_receiver(), name="ws_receiver"),
                ]
                ...
                for t in done:
                    ...
                    if exc is not None:
                        has_error = True
                        logging.warning(f"🔎 网关任务退出: {t.get_name()} -> {type(exc).__name__}: {exc}")
                        break
                if has_error: 
                    logging.warning("🔄 发现异常断网，5秒后重连...")
                    await asyncio.sleep(5)
                else:
                    logging.warning("🔄 云端连接已结束，5秒后重连...")
                    await asyncio.sleep(5)
